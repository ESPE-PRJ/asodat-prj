...
class AuditoriasSocios extends Page implements HasTable
{
    use InteractsWithTable;

    protected static ?string $navigationIcon = 'heroicon-o-document-text';

    protected static string $view = 'filament.pages.auditorias-socios';

    protected static ?string $title = 'Auditorías de Socios';

    protected static ?string $navigationLabel = 'Auditorías de Socios';
    ...
    public function table(Table $table): Table
    {
        return $table
            ->query(
                Audit::query()
                    ->where('auditable_type', 'App\\Models\\Socio')
                    ->with(['user'])
                    ->orderBy('created_at', 'desc')
            )
            ->columns([
                TextColumn::make('socio_nombre')
                    ->label('Socio Afectado')
                    ->getStateUsing(function (Audit $record) {
                        $socio = \App\Models\Socio::find($record->auditable_id);
                        return $socio ? $socio->apellidos_nombres : 'Socio no encontrado (ID: ' . $record->auditable_id . ')';
                    })
                    ->searchable()
                    ->sortable(false)
                    ->wrap(),
            ...
            ->filters([
                ...
                SelectFilter::make('user_id')
                    ...
                    ->options(function () {
                        return \App\Models\User::whereHas('audits', function ($query) {
                            $query->where('auditable_type', 'App\\Models\\Socio');
            ...
            ->actions([
                Tables\Actions\ViewAction::make()
                    ...
                    ->modalContent(function (Audit $record) {
                        ...
                        $content .= '<div><strong class="text-gray-700 dark:text-gray-300">Socio afectado:</strong><br><span class="text-gray-900 dark:text-white">' . ($socio ? $socio->apellidos_nombres : 'Socio no encontrado') . '</span></div>';
            ...
            ->emptyStateHeading('No hay auditorías registradas')
            ->emptyStateDescription('Las auditorías aparecerán aquí cuando se realicen cambios en los registros de socios.')
